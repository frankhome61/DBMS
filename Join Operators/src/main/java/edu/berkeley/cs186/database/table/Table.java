...
public class Table implements Iterable<Record>, Closeable {
  ...
  public class RIDPageIterator implements BacktrackingIterator<RecordId> {
      ...
      public boolean hasNext() {
        for (int i = this.currentIndex; i < getNumRecordsPerPage(); ++i) {
      ...
      public RecordId next() {
          Bits.Bit currBit = Bits.getBit(this.bitMap, this.currentIndex);
          while (currBit == Bits.Bit.ZERO) {
              this.currentIndex++;
              currBit = Bits.getBit(this.bitMap, this.currentIndex);
          }
          RecordId newRec = new RecordId(currPage.getPageNum(), this.currentIndex);
          this.currentIndex++;
          return newRec;
      }
      ...
      public void reset() {
        ...
        this.currentIndex = (short) (this.markedIndex - 1);
      }
  }
  ...
  public class RIDBlockIterator implements BacktrackingIterator<RecordId> {
    ...
    public RIDBlockIterator(BacktrackingIterator<Page> block) {
      this.block = block;
      if (block.hasNext()) {
        Page nextPage = block.next();
        this.blockIter = new RIDPageIterator(nextPage);
      }
