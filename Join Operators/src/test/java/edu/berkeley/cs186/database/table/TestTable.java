...
public class TestTable {
  ...
  public void testEmptyTableIterator() throws DatabaseException {
    BacktrackingIterator<Record> iter = table.iterator();
    assertFalse(iter.hasNext());
    // Reload the database and iterate again.
    table.close();
  ...
  @Test
  public void testEmptyRIDBlockIterator() {
      Iterator<Page> pages = table.getAllocator().iterator();
      pages.next();
      BacktrackingIterator<Record> iter = new RecordIterator(table, table.new RIDBlockIterator(pages, 3));
      assertFalse(iter.hasNext());
  ...
  @Test (expected = NoSuchElementException.class)
  public void testEmptyRIDBlockIteratorWithException() {
    Iterator<Page> pages = table.getAllocator().iterator();
    pages.next();
    BacktrackingIterator<Record> iter = new RecordIterator(table, table.new RIDBlockIterator(pages, 3));
    iter.next();
    ...
    @Test
    public void testRIDBlockIterator2() throws DatabaseException {
      int numRecords = setupIteratorTest(3);
      Iterator<Page> pages = table.getAllocator().iterator();
      // Header page.
      pages.next();
      BacktrackingIterator<Record> iter = new RecordIterator(table, table.new RIDBlockIterator(pages, 3));
      assertFalse(iter.next() == null);
      checkSequentialRecords(1, numRecords, 1, iter);
      assertFalse(iter.hasNext());
    }

    @Test (expected = NoSuchElementException.class)
    public void testEmptyRIDBlockIteratorResetMark() throws DatabaseException {
      Iterator<Page> pages = table.getAllocator().iterator();
      pages.next();
      BacktrackingIterator<Record> iter = new RecordIterator(table, table.new RIDBlockIterator(pages, 3));
      assertFalse(iter.hasNext());
