...
public class Histogram {
  ...
  public void buildHistogram(Table table, int attribute){
      ...
      // Calculate the min and max values of the entire histogram
      this.minValue = Integer.MAX_VALUE;
      this.maxValue = Integer.MIN_VALUE;
      for (Record r: table) {
          float quantData = quantization(r, attribute);
          if (quantData < this.minValue) {
              this.minValue = quantData;
          }
          if (quantData > this.maxValue) {
              this.maxValue = quantData;
          }
      }

      // Calculate the width of the buckets and instantiate the buckets
      this.width = (int) Math.ceil((double) (this.maxValue - this.minValue) / (double) this.numBuckets);
      for (int i = 0; i < this.numBuckets; i++) {
          this.buckets[i] = new Bucket<>(this.minValue + i * this.width,
                  this.minValue + (i + 1) * width);
      }

      for (Record r: table) {
          float quantData = quantization(r, attribute);
          int buckInd = bucketIndex(quantData);
          buckets[buckInd].increment(quantData);
      }
  }
  ...
  private float [] allEquality(float qvalue){
    float [] result = new float[this.numBuckets];
    int buckInd = bucketIndex(qvalue);
    if (buckInd > this.buckets.length - 1 || buckInd < 0) {
        return result;
    }
    Bucket<Float> buck = this.buckets[buckInd];
    float scale = (float) (1.0 / buck.getDistinctCount());
    result[buckInd] = scale;
    return result;
  }
 ...
 /**
  ...
  private float [] allNotEquality(float qvalue){
    float [] result = new float[this.numBuckets];
      ...
      Bucket<Float> buck = this.buckets[buckInd];
      float scale = (float) (1.0 - 1.0 / buck.getDistinctCount());
      result[buckInd] = scale;
    return result;
  }
  ...
  private float [] allGreaterThan(float qvalue){
    ...
    float [] result = new float[this.numBuckets];
      int buckInd = bucketIndex(qvalue);
      if (buckInd > this.buckets.length - 1) {
          return result;
      }
      ...
      Bucket<Float> buck = this.buckets[buckInd];
      float end = buck.getEnd();
      float scale = (end - qvalue) / this.width;
      result[buckInd] = scale;
      for (int i = buckInd + 1; i < numBuckets; i++) {
          result[i] = 1;
      }
    return result;
  }
 ...
 /**
  ...
  private float [] allLessThan(float qvalue){
      ...
      //Everything is less than qvalue
      if (buckInd > this.buckets.length - 1) {
      ...
      //Everything is greater than qvalue
      if (buckInd < 0) {
          return result;
      }

      Bucket<Float> buck = this.buckets[buckInd];
      float start = buck.getStart();
      float scale = (qvalue - start) / this.width;
      result[buckInd] = scale;
      for (int i = 0; i < buckInd; i++) {
          result[i] = 1;
      }
      return result;
  }
