...
class InnerNode extends BPlusNode {
  ...
  public Optional<Pair<DataBox, Integer>> put(DataBox key, RecordId rid)
      ...
      BPlusNode childNode = getChild(locationIndex);
      Optional<Pair<DataBox, Integer>> returnedNode = childNode.put(key, rid);
      ...
      Pair<DataBox, Integer> returnData = returnedNode.get();
      children.add(locationIndex + 1, returnedNode.get().getSecond());
      keys.add(locationIndex, returnData.getFirst());

      if (keys.size() <= 2 * d) {
          sync();
          return Optional.empty();
      }
      //Child overflows but current InnerNode is not full
      List<DataBox> leftHalfKeys = keys.subList(0, d);
      List<DataBox> rightHalfKeys = keys.subList(d + 1, 2 * d + 1);
      List<Integer> lChildren = children.subList(0, d + 1);
      List<Integer> rChildren = children.subList(d + 1, 2 * d + 2);

      InnerNode newNode = new InnerNode(metadata, rightHalfKeys, rChildren);
  ...
  public Optional<Pair<DataBox, Integer>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
      ...
      while (data.hasNext()) {
          int d = metadata.getOrder(); // the order of the B+ Tree
          int locationIndex = children.size() - 1;
          BPlusNode childNode = getChild(locationIndex);
          Optional<Pair<DataBox, Integer>> returnedNode = childNode.bulkLoad(data, fillFactor);
          if (!returnedNode.isPresent()) {
              //Child overflow doesn't occur
              sync();
              return Optional.empty();
          }
          ...
          if (keys.size() > 2 * d) {
              //Child overflows but current InnerNode is full
