...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
      ...
      Iterator<RecordId> iter = new BPlusTreeIterator(currNode, key);
      return iter;
    }
    ...
    public void put(DataBox key, RecordId rid) throws BPlusTreeException {
        typecheck(key);
        Optional<Pair<DataBox, Integer>> returnNode = this.root.put(key, rid);
        if (returnNode.isPresent()) {
            int pageNum = returnNode.get().getSecond();

            List<DataBox> newData = new ArrayList<>();
            List<Integer> newChildren = new ArrayList<>();

            newChildren.add(this.root.getPage().getPageNum());
            newChildren.add(pageNum);
            newData.add(returnNode.get().getFirst());

            InnerNode newNode = new InnerNode(metadata, newData, newChildren);
            this.root = newNode;
            writeHeader(headerPage.getByteBuffer());
        }
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) throws BPlusTreeException {
        if (!this.root.getLeftmostLeaf().getKeys().isEmpty() || !this.root.getLeftmostLeaf().equals(this.root)) {
            throw new BPlusTreeException("Tree is not empty.");
        }
        while (data.hasNext()) {
            Optional<Pair<DataBox, Integer>> returnNode = this.root.bulkLoad(data, fillFactor);
            if (returnNode.isPresent()) {
                int pageNum = returnNode.get().getSecond();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
      // TODO(hw2): Add whatever fields and constructors you want here.
        ...
        public BPlusTreeIterator() {
            curr = BPlusTree.this.root.getLeftmostLeaf();
            ridIter = curr.scanAll();
        }

        public BPlusTreeIterator(LeafNode currNode, DataBox key) {
            curr = currNode.getLeftmostLeaf();
            ridIter = curr.scanGreaterEqual(key);
        }
        @Override
        public boolean hasNext() {
            ...
            Optional<LeafNode> sibling = curr.getRightSibling();
            LeafNode siblingNode = sibling.get();
            if (siblingNode.scanAll().hasNext()) {
                return true;
            }
            return false;
        }
