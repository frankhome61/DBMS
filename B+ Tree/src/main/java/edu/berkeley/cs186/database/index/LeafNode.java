...
class LeafNode extends BPlusNode {
  ...
  public Optional<Pair<DataBox, Integer>> put(DataBox key, RecordId rid)
      ...
      int d = metadata.getOrder(); // the order of the B+ Tree
      if (keys.contains(key)) {
          throw new BPlusTreeException("Sorry, duplicate entry not allowed. Key " + key + " is duplicate");
      }
      int locationIndex = InnerNode.numLessThanEqual(key, keys);
      keys.add(locationIndex, key);
      rids.add(locationIndex, rid);
      ...
      //If overflow occurs
      List<DataBox> leftHalfKeys = keys.subList(0, d);
      List<RecordId> leftHalfRids = rids.subList(0, d);

      List<DataBox> rightHalfKeys = keys.subList(d, keys.size());
      List<RecordId> rightHalfRids = rids.subList(d, keys.size());

      this.keys = leftHalfKeys;
      this.rids = leftHalfRids;
      LeafNode newNode = new LeafNode(metadata, rightHalfKeys, rightHalfRids, rightSibling);

      int right_node_page_num = newNode.getPage().getPageNum();
      this.rightSibling = Optional.of(right_node_page_num);

      DataBox splitKey = rightHalfKeys.get(0);
      Pair newPair = new Pair(splitKey, right_node_page_num);
      sync();
  ...
  public Optional<Pair<DataBox, Integer>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
      ...
      int d = metadata.getOrder(); // the order of the B+ Tree
      int numToFill = (int) Math.ceil(2 * d * fillFactor);
      while (data.hasNext()) {
          Pair<DataBox, RecordId> dataPair = data.next();
          DataBox key= dataPair.getFirst();
          RecordId rid = dataPair.getSecond();
          ...
          //Overflow occurs
          if (keys.size() > numToFill) {
              //If overflow occurs
              List<DataBox> leftHalfKeys = keys.subList(0, keys.size() - 1);
              List<RecordId> leftHalfRids = rids.subList(0, keys.size() - 1);
              List<DataBox> rightHalfKeys = keys.subList(keys.size() - 1, keys.size());
              List<RecordId> rightHalfRids = rids.subList(keys.size() - 1, keys.size());
              LeafNode newNode = new LeafNode(metadata, rightHalfKeys, rightHalfRids, rightSibling);
              this.keys = leftHalfKeys;
              this.rids = leftHalfRids;
              int right_node_page_num = newNode.getPage().getPageNum();
  ...
  public static LeafNode fromBytes(BPlusTreeMetadata metadata, int pageNum) {
      ...
      int siblingVal = buf.getInt();
      Optional<Integer> rightSibling;
      if (siblingVal == -1) {
          rightSibling = Optional.empty();
      } else {
          rightSibling = Optional.of(siblingVal);
      }
